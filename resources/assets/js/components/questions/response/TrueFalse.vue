    ...
    <div>
        <div class="form-group">
            <div class="form-check" v-for="(option, key) in selectOptions">
                <input class="form-check-input" :disabled="disabled" type="radio"
                    v-bind:id="'radio'+question.id + '_' + key" v-model="selected" :value="option">
...
<script>
    export default {
        ...
        watch: {
            selected: function(newValue, value) {
                if (newValue !== null && newValue !== value && !(!this.create_new_response && this.response && this.response
                    .response_info && newValue == this.response.response_info.choice)) {
                const response = {
    ...
    mounted() {
        if (this.response && this.response.hasOwnProperty('response_info') && this.response.response_info
            .hasOwnProperty('choice')) {
            this.selected = this.response.response_info.choice;
        }
    }
    }
