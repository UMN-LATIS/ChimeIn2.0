        ...
        mounted: function () {
            ...
            Echo.join('session-status.' + this.chimeId)
                .listen('StartSession', m => {
                    ...
                    this.$announcer.set("A new question has been open.  There are " + this.sessions.length +
                        " questions open");
                })
                .listen('EndSession', m => {
                    var removeIndex = this.sessions.findIndex(session => session.id == m.session.id);
                    this.sessions.splice(removeIndex, 1);
                    this.$announcer.set("A question has been closed.  There are " + this.sessions.length +
                        " questions open");
                });
